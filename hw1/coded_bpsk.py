...
for SNRdb in range(startSNRdB,endSNRdB,SNRdBstep):
    ...
    while(biterrorcount < maxbiterrorcount):
        ...
        if not np.all(syndrome == 0):
            ...
            decoded[2] = received[2] ^ received[3];
            decoded[1] = received[1] ^ received[2] ^ received[3];
            decoded[0] = received[0] ^ received[1] ^ received[2];
...
plt.legend(["Simulated Coded BPSK", "Theortical Bit Error for Uncoded"], loc="upper right")
plt.xlabel("Eb/N0  (dB)")
plt.ylabel("Pb")
plt.show()
