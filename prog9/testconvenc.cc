...
int main()
{
   ...
   // Fourth test: Test the IIR stuff
   int k4 = 1;					// two inputs
   int n4 = 2;					// three outputs
   // G = [1  (1+D+D^2/(1+D^2)]
   unsigned int g4num[] = {7};	// third column of G: D and D^2
   unsigned int g4den = 5;		    // =101 denominator  1+D^2
   int p4 = 2;				// degree of denominators
   BinConvIIR conv4(k4,n4,p4,g4num,g4den);
   int in4 = 1;
   cout << "Fourth Test: " << endl;
   printconvtest(conv4,in4, Nimpulse);
   cout << endl << endl << endl;
