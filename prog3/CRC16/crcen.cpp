...
int main (int argc, char** argv)
{
   ...
   if (argc != 4)
   {
      ...
      std::cout << "Usage: `crcenc K filein fileout`" << std::endl;
      return -1;
   }
   ...
   for (size_t i = 0; i < buffer.size(); i += block_len)
   {
      ...
      uint32_t result = crc.CRC(&bitArray[i * 8], static_cast<int>(current_block_len * 8));
      printf("current block len is %ld\n", current_block_len);
      for(int j = 0; j < current_block_len*8; j++)
      {
         printf("bit Array[%d] is %d\n", j, bitArray[j]);
      }
      printf("current block len is %ld\n", current_block_len);
