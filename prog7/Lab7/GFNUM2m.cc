...
// e.g. g = 0x13 = 1 0011 = D^4 + D + 1
{
   ...
   v2p[0] = -1;  //No element at 0
   p2v[gfN] = 1; //back at 1 at last element
   p2v[0] = 1; //always the case
   uint32_t result;
   for (int i = 0; i < gfN+1; i++)
   {
      if (i == 0)
      {
         result = 1;
         v2p[i] = -1; //no element at 0
         p2v[0] = 1; //always the case
      }
      else
      {
         result = result << 1; //next power up
         if (result >= (1<<m))
         {
            result ^= g;
         }
